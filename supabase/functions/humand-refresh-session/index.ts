...
serve(async (req) => {
  ...
  try {
    ...
    const seccionNombres = segmentaciones
      .filter((s) => GRUPOS.includes(s.group))
      .map((s) => s.item)
      .filter(Boolean)

    const seccion =
      segmentaciones.find((s) => s.group === 'SECCIONES')?.item ??
      segmentaciones.find((s) => s.group === 'DEPARTAMENTOS')?.item ??
      segmentaciones.find((s) => s.group === 'GERENCIAS')?.item ??
      ''

    const jefeInternalId =
      userData.relationships?.find((r) => r.name === 'BOSS')?.employeeInternalId ?? null
    ...
    const seccionIds = Array.isArray(mapData)
      ? mapData.map((r) => r.segmentation_item_id)
      : []
